...
if bytes>2^21
    msgbox('The total size of roms is greater than 2MB !')
    return
end
...
for i=1:1:heigth
    ...
    if rom_size==2^20;
        if not(isempty(pos1024ko));
            if mean(dumb2Mo(pos1024ko(end)))==255
    ...
    if rom_size==2^19;
        if not(isempty(pos512ko));
            if mean(dumb2Mo(pos512ko(end)))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
    ...
    if rom_size==2^18;
        if not(isempty(pos256ko));
            if mean(dumb2Mo(pos256ko(end)))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
    ...
    if rom_size==2^17;
        if not(isempty(pos128ko));
            if mean(dumb2Mo(pos128ko(end)))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
    ...
    if rom_size==2^16;
        if not(isempty(pos64ko));
            if mean(dumb2Mo(pos64ko(end)))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
    ...
    if rom_size==2^15;
        if not(isempty(pos32ko));
            if mean(dumb2Mo(pos32ko(end)))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
