...
fclose(fid);
dumb2Mo(1:2^15)=a;%place the mandatory multiboot rom at the beginning
...
for i=1:1:heigth
    ...
    if rom_size==2^20;
        if not(isempty(pos1024ko));
            if mean(dumb2Mo(pos1024ko(end):pos1024ko(end)+rom_size-1))==255
                ...
                fclose(fid);
                games=games+1;
                dumb2Mo(pos1024ko(end):pos1024ko(end)+rom_size-1)=a;
    ...
    if rom_size==2^19;
        if not(isempty(pos512ko));
            if mean(dumb2Mo(pos512ko(end):pos512ko(end)+rom_size-1))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
                ...
                fclose(fid);
                games=games+1;
                dumb2Mo(pos512ko(end):pos512ko(end)+rom_size-1)=a;
                disp('File added to the Multiboot rom')
                %now disable any adresses higher than this range
    ...
    if rom_size==2^18;
        if not(isempty(pos256ko));
            if mean(dumb2Mo(pos256ko(end):pos256ko(end)+rom_size-1))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
                ...
                fclose(fid);
                games=games+1;
                dumb2Mo(pos256ko(end):pos256ko(end)+rom_size-1)=a;
                disp('File added to the Multiboot rom')
                %now disable any adresses higher than this range
    ...
    if rom_size==2^17;
        if not(isempty(pos128ko));
            if mean(dumb2Mo(pos128ko(end):pos128ko(end)+rom_size-1))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
                ...
                fclose(fid);
                games=games+1;
                dumb2Mo(pos128ko(end):pos128ko(end)+rom_size-1)=a;
                disp('File added to the Multiboot rom')
                %now disable any adresses higher than this range
    ...
    if rom_size==2^16;
        if not(isempty(pos64ko));
            if mean(dumb2Mo(pos64ko(end):pos64ko(end)+rom_size-1))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
                ...
                fclose(fid);
                games=games+1;
                dumb2Mo(pos64ko(end):pos64ko(end)+rom_size-1)=a;
                disp('File added to the Multiboot rom')
                %now disable any adresses higher than this range
    ...
    if rom_size==2^15;
        if not(isempty(pos32ko));
            if mean(dumb2Mo(pos32ko(end):pos32ko(end)+rom_size-1))==255
                fid=['./rom/',char(listing(i).name)];
                disp(['Slot available for ',fid])
                fid = fopen(fid,'r');
                while ~feof(fid)
                ...
                fclose(fid);
                games=games+1;
                dumb2Mo(pos32ko(end):pos32ko(end)+rom_size-1)=a;
                disp('File added to the Multiboot rom')
                %disable itself
